...
if (isProduction) {
  console.error('⛔ SECURITY ALERT: Bulk auth operations are strictly blocked on non-local environments!');
  process.exit(1);
}

// 2. Require confirmation phrase
if (process.argv[2] !== 'CONFIRM_BULK_RESET_OPERATION') {
  console.error('⛔ ERROR: You must pass the confirmation phrase "CONFIRM_BULK_RESET_OPERATION" as the first argument.');
  process.exit(1);
}
...
async function main() {
  console.log('Resetting all passwords for testing...');
