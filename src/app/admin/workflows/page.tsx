...
import { SystemHealth } from '../../../components/admin/sections/SystemHealth';
...
export default function OperationsHubPage() {
  const [activeTab, setActiveTab] = React.useState<'automations' | 'health'>('automations');
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-[13px] text-slate-500 mt-1">Global command for System health, background automations, and data pipelines.</p>
          </div>
          
          <div className="flex items-center gap-3">
            <button className="flex items-center justify-center gap-2 px-6 py-3 bg-white border border-slate-200 text-slate-600 text-[12px] font-bold rounded-xl hover:bg-slate-50 transition-all">
              System Logs
            </button>
        ...
        <div className="flex items-center gap-8 border-b border-slate-100">
          {[
            { id: 'automations', label: 'Background Automations', icon: Zap },
            { id: 'health', label: 'Platform Health', icon: HealthIcon },
          ].map((tab) => (
            <button
              key={tab.id}
